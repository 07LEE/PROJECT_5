...
def get_novel_info(save=True):
    ...
    genres = {'로맨스': 101, '로판': 109, '판타지': 102, '현판': 110, '무협': 103}
    novel_info_list = []
    ...
    for genre, urls in links_per_novel.items():
        for url in urls:
            ...
            if response.status_code == 200:
                ...
                novel['id'] = re.search(r'\d{6,}', url).group(0)  # id 추가
                novel['title'] = dom.select_one('title').text  # 제목 추가
                novel['writer'] = dom.select_one(".info_area a").text  # 작가 추가
                novel['genre'] = genre  # genre 추가
                # 전체 회차 수 추가
                novel['total_epi'] = re.search(r'\d+', dom.select_one('.past_number').text).group(0)

                # 발화자 유무 추가
                response = requests.get(
                    f"https://novel.naver.com/webnovel/detail?novelId={novel_id}&volumeNo=1")
                if response.status_code == 200:
                    dom = BeautifulSoup(response.text, 'html.parser')
...
def get_download(novel_info_list, labeled_only=False):
    '''
    ...
    for novel in tqdm(novel_info_list):
        novel_id = novel['id']
        total_epi = novel['total_epi']

        # 레이블/언레이블 데이터를 모두 수집하는 옵션
        if labeled_only is False:
            for epi in range(1, 1+int(total_epi)):
                ...
                if response.status_code == 200:
                    ...
                    # 파일명 = id_특문 제거된 제목
                    filename = novel_id + "_" + re.sub('[\\/:*?"<>|]', "", dom.select_one('title').text)
