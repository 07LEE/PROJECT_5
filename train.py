...
if MODEL_NAME == 'CSN':
    ...
    try:
        ...
        train_data = build_data_loader(train_file, alias2id, args, skip_only_one=True,
                                       save_filename='save_data/train_data', MODEL_NAME=MODEL_NAME)
        dev_data = build_data_loader(
            dev_file, alias2id, args, save_filename='save_data/dev_data', MODEL_NAME=MODEL_NAME)
        test_data = build_data_loader(
            test_file, alias2id, args, save_filename='save_data/test_data', MODEL_NAME=MODEL_NAME)
        print("---------------------------------------------------------")
    ...
    try:
        ...
        train_data = build_data_loader(train_file, alias2id, args, skip_only_one=True,
                                       save_filename='save_data/train_Kdata', MODEL_NAME=MODEL_NAME)
        dev_data = build_data_loader(
            dev_file, alias2id, args, save_filename='save_data/dev_Kdata', MODEL_NAME=MODEL_NAME)
        test_data = build_data_loader(
            test_file, alias2id, args, save_filename='save_data/test_Kdata', MODEL_NAME=MODEL_NAME)
        print("---------------------------------------------------------")
...
if MODEL_NAME == 'CSN':
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for i, (_, CSSs, sent_char_lens, mention_poses, quote_idxes, cut_css, one_hot_label, true_index, _) in enumerate(tqdm(train_data)):
            ...
            try:
                features, tokens_list = convert_examples_to_Kfeatures(
                    examples=CSSs, tokenizer=tokenizer)
                scores, scores_false, scores_true = model(
