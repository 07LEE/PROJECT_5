...
from model.model import CSN, KCSN
...
if MODEL_NAME == 'CSN':
    ...
    try:
        ...
        train_data = build_data_loader(train_file, alias2id, args, skip_only_one=True, save_filename='save_data/train_data', MODEL_NAME=MODEL_NAME)
        dev_data = build_data_loader(dev_file, alias2id, args, save_filename='save_data/dev_data', MODEL_NAME=MODEL_NAME)
        test_data = build_data_loader(test_file, alias2id, args, save_filename='save_data/test_data', MODEL_NAME=MODEL_NAME)
        print("---------------------------------------------------------")
    ...
    try:
        print('load_data_loader ----------------------------------------')
        train_data = load_data_loader('save_data/train_Kdata')
        dev_data = load_data_loader('save_data/dev_Kdata')
        test_data = load_data_loader('save_data/test_Kdata')
        example_print = True

    except FileNotFoundError:
        train_data = build_data_loader(train_file, alias2id, args, skip_only_one=True, save_filename='save_data/train_Kdata', MODEL_NAME=MODEL_NAME)
        dev_data = build_data_loader(dev_file, alias2id, args, save_filename='save_data/dev_Kdata', MODEL_NAME=MODEL_NAME)
        test_data = build_data_loader(test_file, alias2id, args, save_filename='save_data/test_Kdata', MODEL_NAME=MODEL_NAME)
        print("---------------------------------------------------------")
    ...
    if example_print is not False:
        ...
        print('DEV EXAMPLE ---------------------------------------------')
        dev_test_iter = iter(dev_data)
        _, CSSs, sent_char_lens, mention_poses, quote_idxes, cut_css, one_hot_label, true_index, _ = next(dev_test_iter)
        print('Candidate-specific segments : ', CSSs)
        print('Nearest mention positions : ', mention_poses)

        print('TEST EXAMPLE ---------------------------------------------')
        test_test_iter = iter(test_data)
        _, CSSs, sent_char_lens, mention_poses, quote_idxes, cut_css, one_hot_label, true_index, _ = next(test_test_iter)
        print('Candidate-specific segments : ', CSSs)
        print('Nearest mention positions : ', mention_poses)
        print("---------------------------------------------------------")
...
if MODEL_NAME == 'CSN':
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for i, (_, CSSs, sent_char_lens, mention_poses, quote_idxes, cut_css, one_hot_label, true_index, _) in enumerate(tqdm(train_data)):
            ...
            try:
                features, tokens_list = convert_examples_to_Kfeatures(examples=CSSs, tokenizer=tokenizer)
